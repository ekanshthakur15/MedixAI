...
class MedicalReportsByTreatmentAPIView(APIView):
    ...
    def get(self, request, treatment_name):
        ...
        medical_reports = MedicalReport.objects.filter(user = user , treatment = treatment)
...
class AttributesListByTreatment(APIView):
    def get(self, request):
        ...
        report_attributes = ReportAttributes.objects.filter(medical_report__user = user)

        # Step 5: Extract unique 'element' values
        unique_elements = report_attributes.values_list('element', flat=True).distinct()
...
class MedicalReportAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            files = [upload_to_gemini(medical_report.report.path, mime_type="application/pdf")]
    ...
    def get(self, request, report_id):
        ...
        try:
            ...
            for attribute in query_set:
                ...
                try:
                    low = float(attribute.reference_low) if attribute.reference_low not in [None, ""] else 0
                    high = float(attribute.reference_high) if attribute.reference_high not in [None, ""] else math.inf
...
class MedicalReportAttributeVisualisation(APIView):
    ...
    def get(self, request, element):
        ...
        if start_date_str:
            try:
                start_date = date.fromisoformat(start_date_str)  # Converts string to date
            except ValueError:
        ...
        if end_date_str:
            try:
                end_date = date.fromisoformat(end_date_str)  # Converts string to date
            except ValueError:
        ...
        for medical_report in medical_reports:
            ...
            if attribute:
                ...
                print(f"Attribute not present for {medical_report.name} on {medical_report.test_date}")
